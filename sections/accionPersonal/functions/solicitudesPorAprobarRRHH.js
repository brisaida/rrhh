...
$(document).ready(function () {
    VerSolicitudesPendientesPorAprobar()
});
...
$('#solicitudes').on('click', '.btn-aprobar', function () {
    ...
    accionSeleccionada.estado = 3; // Estado para "aprobado N2"
    Swal.fire({
        title: '¿Estás seguro?',
...
function VerSolicitudesPendientesPorAprobar() {
    $.ajax({
        type: "POST",
        url: "./sections/accionPersonal/controller/solicitudesPendientesPorAprobar.php",
        data: {
        },
        error: function (error) {
            Swal.fire({
